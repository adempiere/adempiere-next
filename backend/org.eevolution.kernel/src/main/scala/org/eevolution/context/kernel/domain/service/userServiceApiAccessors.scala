...
trait userServiceApiAccessors {
  def getById(id: Id): RIO[UserService with UserRepository, Option[User]] =
  ...
  def getUser(
      ...
      userName: String,
      accountEmail: String
  ): RIO[UserService with UserRepository, Option[User]] =
    ZIO.accessM(_.get.getUser(partnerId, userName, accountEmail))

  def save(user: User): RIO[UserService with UserRepository, User] =
    ZIO.accessM(_.get.save(user))
