...
import io.getquill.{Literal, OracleJdbcContext}
...
package object oracleContextApi {

  type OracleContext = Has[OracleContext.Service]

  object OracleContext {

    class Service(
        dataSource: DataSource
    ) extends OracleJdbcContext(
          Literal,
          dataSource.asInstanceOf[HikariDataSource]
        )
        with DatabaseContext.Service[
