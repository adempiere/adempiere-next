...
trait entityRepositoryApiBase[-R] {

  def getById(id: Id): RIO[R, Option[Entity]]

  def getByEntityType(entityType: EntityType): RIO[R, List[Entity]]

  def getAll: RIO[R, List[Entity]]

  def getAllByClientId(tenantId: TableDirect): RIO[R, List[Entity]]

  def save(entity: Entity): RIO[R, Entity]
}
