...
trait attributeServiceApiAccessors {
  def getById(
      id: Id
  ): RIO[AttributeService with AttributeRepository, Option[Attribute]] =
    RIO.accessM(_.get.getById(id))

  def getByEntity(
      entity: Entity
  ): RIO[AttributeService with AttributeRepository, List[Attribute]] =
    RIO.accessM(_.get.getByEntity(entity))

  def getAll: RIO[AttributeService with AttributeRepository, List[Attribute]] =
  ...
  def save(
      attribute: Attribute
  ): RIO[AttributeService with AttributeRepository, Attribute] =
