...
import org.eevolution.context.dictionary.domain.service.entityTypeServiceBase
import zio.ZLayer

package object entityTypeServiceModule extends entityTypeServiceBase {
  private case class Live(
      entityTypeRepository: EntityTypeRepository.Service
  ) extends Service(entityTypeRepository)

  object EntityTypeServiceLayer {
    val live: ZLayer[EntityTypeRepository, Nothing, EntityTypeService] =
      ZLayer.fromService(entityTypeRepository => Live(entityTypeRepository))
  }
