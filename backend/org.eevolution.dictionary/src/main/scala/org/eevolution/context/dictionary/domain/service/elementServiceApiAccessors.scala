...
trait elementServiceApiAccessors {
  def getById(
      id: Id
  ): RIO[ElementService with ElementRepository, Option[Element]] =
    RIO.accessM(_.get.getById(id))

  def getAll: RIO[ElementService with ElementRepository, List[Element]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(
      tenantId: TableDirect
  ): RIO[ElementService with ElementRepository, List[Element]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))

  def save(
      element: Element
  ): RIO[ElementService with ElementRepository, Element] =
