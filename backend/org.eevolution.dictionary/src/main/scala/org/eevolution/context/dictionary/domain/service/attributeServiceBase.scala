...
trait attributeServiceBase {
  abstract class Service(
      attributeRepository: AttributeRepository.Service
  ) extends AttributeService.Service {

    override def getById(id: Id): RIO[AttributeRepository, Option[Attribute]] =
      attributeRepository.getById(id)

    override def getByEntity(
        entity: Entity
    ): RIO[AttributeRepository, List[Attribute]] =
      attributeRepository.getByEntity(entity)
    ...
    override def getAllByClientId(
      ...
      attributeRepository.getAllByClientId(tenantId)

    override def save(
        attribute: Attribute
    ): RIO[AttributeRepository, Attribute] =
