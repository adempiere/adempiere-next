...
/**
* Accessors method Trait for WfActivity Repository 
*
...
*/
trait wfActivityRepositoryApiAccessors {

  def getByWfNodeId(wfNodeId: TableDirect): RIO[WfActivityRepository, List[WfActivity]] =
   RIO.accessM(_.get.getByWfNodeId(wfNodeId))
   
  def getByWfActivityId(wfActivityId: Id): RIO[WfActivityRepository, Option[WfActivity]] =
   RIO.accessM(_.get.getByWfActivityId(wfActivityId))
   

  def getAll: RIO[WfActivityRepository, List[WfActivity]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfActivityRepository, List[WfActivity]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
  def save(wfActivity: WfActivity): RIO[WfActivityRepository, WfActivity] = 
