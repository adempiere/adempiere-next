...
/**
* Accessors method Trait for WfBlock Repository 
*
...
*/
trait wfBlockRepositoryApiAccessors {

  def getByDescription(description: Option[String]): RIO[WfBlockRepository, List[WfBlock]] =
   RIO.accessM(_.get.getByDescription(description))
   
  def getByWorkflowId(workflowId: TableDirect): RIO[WfBlockRepository, List[WfBlock]] =
   RIO.accessM(_.get.getByWorkflowId(workflowId))
   
  def getByName(name: String): RIO[WfBlockRepository, Option[WfBlock]] =
   RIO.accessM(_.get.getByName(name))
   
  def getByWfBlockId(wfBlockId: Id): RIO[WfBlockRepository, Option[WfBlock]] =
   RIO.accessM(_.get.getByWfBlockId(wfBlockId))
   

  def getAll: RIO[WfBlockRepository, List[WfBlock]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfBlockRepository, List[WfBlock]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
  def save(wfBlock: WfBlock): RIO[WfBlockRepository, WfBlock] = 
