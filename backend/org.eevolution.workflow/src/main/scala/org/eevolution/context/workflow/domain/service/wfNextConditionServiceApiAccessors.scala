...
/*
* Accessors method Trait for WfNextCondition Domain Service 
*
...
*/
trait wfNextConditionServiceApiAccessors {

  def getByWfNextConditionId(wfNextConditionId: Id): RIO[WfNextConditionService with WfNextConditionRepository, Option[WfNextCondition]] =
   RIO.accessM(_.get.getByWfNextConditionId(wfNextConditionId))
   
  def getByWfNodeNextId(wfNodeNextId: TableDirect): RIO[WfNextConditionService with WfNextConditionRepository, List[WfNextCondition]] =
   RIO.accessM(_.get.getByWfNodeNextId(wfNodeNextId))
   
  def getByValue(value: String): RIO[WfNextConditionService with WfNextConditionRepository, Option[WfNextCondition]] =
   RIO.accessM(_.get.getByValue(value))
   

  def getAll: RIO[WfNextConditionService with WfNextConditionRepository, List[WfNextCondition]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfNextConditionService with WfNextConditionRepository, List[WfNextCondition]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
    def save(wfNextCondition: WfNextCondition): RIO[WfNextConditionService with WfNextConditionRepository, WfNextCondition] =
