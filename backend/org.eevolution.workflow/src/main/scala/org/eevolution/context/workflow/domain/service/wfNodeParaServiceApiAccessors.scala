...
/*
* Accessors method Trait for WfNodePara Domain Service 
*
...
*/
trait wfNodeParaServiceApiAccessors {

  def getByWfNodeId(wfNodeId: TableDirect): RIO[WfNodeParaService with WfNodeParaRepository, List[WfNodePara]] =
   RIO.accessM(_.get.getByWfNodeId(wfNodeId))
   
  def getByDescription(description: Option[String]): RIO[WfNodeParaService with WfNodeParaRepository, List[WfNodePara]] =
   RIO.accessM(_.get.getByDescription(description))
   
  def getByAttributeName(attributeName: Option[String]): RIO[WfNodeParaService with WfNodeParaRepository, List[WfNodePara]] =
   RIO.accessM(_.get.getByAttributeName(attributeName))
   
  def getByWfNodeParaId(wfNodeParaId: Id): RIO[WfNodeParaService with WfNodeParaRepository, Option[WfNodePara]] =
   RIO.accessM(_.get.getByWfNodeParaId(wfNodeParaId))
   

  def getAll: RIO[WfNodeParaService with WfNodeParaRepository, List[WfNodePara]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfNodeParaService with WfNodeParaRepository, List[WfNodePara]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
    def save(wfNodePara: WfNodePara): RIO[WfNodeParaService with WfNodeParaRepository, WfNodePara] =
