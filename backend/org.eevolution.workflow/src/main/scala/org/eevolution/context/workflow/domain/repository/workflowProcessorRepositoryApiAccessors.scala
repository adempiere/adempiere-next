...
/**
* Accessors method Trait for WorkflowProcessor Repository 
*
...
*/
trait workflowProcessorRepositoryApiAccessors {

  def getByDescription(description: Option[String]): RIO[WorkflowProcessorRepository, List[WorkflowProcessor]] =
   RIO.accessM(_.get.getByDescription(description))
   
  def getByName(name: String): RIO[WorkflowProcessorRepository, Option[WorkflowProcessor]] =
   RIO.accessM(_.get.getByName(name))
   
  def getByWorkflowProcessorId(workflowProcessorId: Id): RIO[WorkflowProcessorRepository, Option[WorkflowProcessor]] =
   RIO.accessM(_.get.getByWorkflowProcessorId(workflowProcessorId))
   

  def getAll: RIO[WorkflowProcessorRepository, List[WorkflowProcessor]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WorkflowProcessorRepository, List[WorkflowProcessor]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
  def save(workflowProcessor: WorkflowProcessor): RIO[WorkflowProcessorRepository, WorkflowProcessor] = 
