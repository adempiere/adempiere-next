...
/*
* Accessors method Trait for WfResponsible Domain Service 
*
...
*/
trait wfResponsibleServiceApiAccessors {

  def getByName(name: String): RIO[WfResponsibleService with WfResponsibleRepository, Option[WfResponsible]] =
   RIO.accessM(_.get.getByName(name))
   
  def getByDescription(description: Option[String]): RIO[WfResponsibleService with WfResponsibleRepository, List[WfResponsible]] =
   RIO.accessM(_.get.getByDescription(description))
   
  def getByWfResponsibleId(wfResponsibleId: Id): RIO[WfResponsibleService with WfResponsibleRepository, Option[WfResponsible]] =
   RIO.accessM(_.get.getByWfResponsibleId(wfResponsibleId))
   

  def getAll: RIO[WfResponsibleService with WfResponsibleRepository, List[WfResponsible]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfResponsibleService with WfResponsibleRepository, List[WfResponsible]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
    def save(wfResponsible: WfResponsible): RIO[WfResponsibleService with WfResponsibleRepository, WfResponsible] =
