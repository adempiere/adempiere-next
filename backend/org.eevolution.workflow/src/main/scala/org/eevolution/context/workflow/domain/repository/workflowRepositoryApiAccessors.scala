...
/**
* Accessors method Trait for Workflow Repository 
*
...
*/
trait workflowRepositoryApiAccessors {

  def getByName(name: String): RIO[WorkflowRepository, Option[Workflow]] =
   RIO.accessM(_.get.getByName(name))
   
  def getByDescription(description: Option[String]): RIO[WorkflowRepository, List[Workflow]] =
   RIO.accessM(_.get.getByDescription(description))
   
  def getByWorkflowId(workflowId: Id): RIO[WorkflowRepository, Option[Workflow]] =
   RIO.accessM(_.get.getByWorkflowId(workflowId))
   
  def getByValue(value: String): RIO[WorkflowRepository, Option[Workflow]] =
   RIO.accessM(_.get.getByValue(value))
   
  def getByDocumentNo(documentNo: Option[String]): RIO[WorkflowRepository, List[Workflow]] =
   RIO.accessM(_.get.getByDocumentNo(documentNo))
   

  def getAll: RIO[WorkflowRepository, List[Workflow]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WorkflowRepository, List[Workflow]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
  def save(workflow: Workflow): RIO[WorkflowRepository, Workflow] = 
