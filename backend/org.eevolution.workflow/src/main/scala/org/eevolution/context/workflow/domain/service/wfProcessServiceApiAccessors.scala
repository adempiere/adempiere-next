...
/*
* Accessors method Trait for WfProcess Domain Service 
*
...
*/
trait wfProcessServiceApiAccessors {

  def getByWorkflowId(workflowId: TableDirect): RIO[WfProcessService with WfProcessRepository, List[WfProcess]] =
   RIO.accessM(_.get.getByWorkflowId(workflowId))
   
  def getByWfProcessId(wfProcessId: Id): RIO[WfProcessService with WfProcessRepository, Option[WfProcess]] =
   RIO.accessM(_.get.getByWfProcessId(wfProcessId))
   

  def getAll: RIO[WfProcessService with WfProcessRepository, List[WfProcess]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfProcessService with WfProcessRepository, List[WfProcess]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
    def save(wfProcess: WfProcess): RIO[WfProcessService with WfProcessRepository, WfProcess] =
