...
/*
* Accessors method Trait for WfNodeNext Domain Service 
*
...
*/
trait wfNodeNextServiceApiAccessors {

  def getByWfNodeId(wfNodeId: TableDirect): RIO[WfNodeNextService with WfNodeNextRepository, List[WfNodeNext]] =
   RIO.accessM(_.get.getByWfNodeId(wfNodeId))
   
  def getByWfNextId(wfNextId: Table): RIO[WfNodeNextService with WfNodeNextRepository, List[WfNodeNext]] =
   RIO.accessM(_.get.getByWfNextId(wfNextId))
   
  def getBySeqNo(seqNo: Int): RIO[WfNodeNextService with WfNodeNextRepository, List[WfNodeNext]] =
   RIO.accessM(_.get.getBySeqNo(seqNo))
   
  def getByDescription(description: Option[String]): RIO[WfNodeNextService with WfNodeNextRepository, List[WfNodeNext]] =
   RIO.accessM(_.get.getByDescription(description))
   
  def getByWfNodeNextId(wfNodeNextId: Id): RIO[WfNodeNextService with WfNodeNextRepository, Option[WfNodeNext]] =
   RIO.accessM(_.get.getByWfNodeNextId(wfNodeNextId))
   

  def getAll: RIO[WfNodeNextService with WfNodeNextRepository, List[WfNodeNext]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfNodeNextService with WfNodeNextRepository, List[WfNodeNext]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
    def save(wfNodeNext: WfNodeNext): RIO[WfNodeNextService with WfNodeNextRepository, WfNodeNext] =
