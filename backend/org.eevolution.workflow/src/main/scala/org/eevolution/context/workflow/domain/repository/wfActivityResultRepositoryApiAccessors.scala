...
/**
* Accessors method Trait for WfActivityResult Repository 
*
...
*/
trait wfActivityResultRepositoryApiAccessors {

  def getByWfActivityId(wfActivityId: TableDirect): RIO[WfActivityResultRepository, List[WfActivityResult]] =
   RIO.accessM(_.get.getByWfActivityId(wfActivityId))
   
  def getByAttributeName(attributeName: String): RIO[WfActivityResultRepository, List[WfActivityResult]] =
   RIO.accessM(_.get.getByAttributeName(attributeName))
   
  def getByWfActivityResultId(wfActivityResultId: Id): RIO[WfActivityResultRepository, Option[WfActivityResult]] =
   RIO.accessM(_.get.getByWfActivityResultId(wfActivityResultId))
   
  def getByDescription(description: Option[String]): RIO[WfActivityResultRepository, List[WfActivityResult]] =
   RIO.accessM(_.get.getByDescription(description))
   

  def getAll: RIO[WfActivityResultRepository, List[WfActivityResult]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfActivityResultRepository, List[WfActivityResult]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
  def save(wfActivityResult: WfActivityResult): RIO[WfActivityResultRepository, WfActivityResult] = 
