...
/*
* Accessors method Trait for WfBlock Domain Service 
*
...
*/
trait wfBlockServiceApiAccessors {

  def getByDescription(description: Option[String]): RIO[WfBlockService with WfBlockRepository, List[WfBlock]] =
   RIO.accessM(_.get.getByDescription(description))
   
  def getByWorkflowId(workflowId: TableDirect): RIO[WfBlockService with WfBlockRepository, List[WfBlock]] =
   RIO.accessM(_.get.getByWorkflowId(workflowId))
   
  def getByName(name: String): RIO[WfBlockService with WfBlockRepository, Option[WfBlock]] =
   RIO.accessM(_.get.getByName(name))
   
  def getByWfBlockId(wfBlockId: Id): RIO[WfBlockService with WfBlockRepository, Option[WfBlock]] =
   RIO.accessM(_.get.getByWfBlockId(wfBlockId))
   

  def getAll: RIO[WfBlockService with WfBlockRepository, List[WfBlock]] =
    RIO.accessM(_.get.getAll)

  def getAllByClientId(tenantId: TableDirect): RIO[WfBlockService with WfBlockRepository, List[WfBlock]] =
    RIO.accessM(_.get.getAllByClientId(tenantId))
    
    def save(wfBlock: WfBlock): RIO[WfBlockService with WfBlockRepository, WfBlock] =
    RIO.accessM(_.get.save(wfBlock))  
